...
#include "Vibex_super_system__Syms.h"
...
SuperSystem::SuperSystem(const char *ram_hier_path, int ram_size_words)
    : _ram(ram_hier_path, ram_size_words, 4) {}

int SuperSystem::Main(int argc, char **argv) {
  bool exit_app;
...
std::string SuperSystem::GetIsaString() const {
  const Vibex_super_system &top = _top;
  assert(top.ibex_super_system);

  std::string base = top.ibex_super_system->RV32E ? "rv32e" : "rv32i";

  std::string extensions;
  if (top.ibex_super_system->RV32M)
    extensions += "m";
  ...
  switch (top.ibex_super_system->RV32B) {
    case 0:  // RV32BNone
      break;
...
int SuperSystem::Setup(int argc, char **argv, bool &exit_app) {
  VerilatorSimCtrl &simctrl = VerilatorSimCtrl::GetInstance();

  simctrl.SetTop(&_top, &_top.clk_sys_i, &_top.rst_sys_ni, // &_top.uart_rx_i,
...
bool SuperSystem::Finish() {
  ...
  svSetScope(svGetScopeFromName("TOP.ibex_super_system"));

  std::cout << "\nPerformance Counters" << std::endl
            << "====================" << std::endl;
  std::cout << ibex_pcount_string(false);

  std::ofstream pcount_csv("ibex_super_system_pcount.csv");
